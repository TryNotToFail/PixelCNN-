...
1. Modify the sample function to get the generated images from the model and ensure 
   the generated images are saved to the gen_data_dir(line 20-33)
...
def my_sample(model, gen_data_dir, sample_batch_size = 25, obs = (3,32,32), sample_op = sample_op):
    ...
    for label in my_bidict:
        ...
        class_labels = torch.zeros(sample_batch_size, dtype=torch.long, device=device)
...
if __name__ == "__main__":
    ...
    model = PixelCNN(nr_resnet=3, nr_filters=100, input_channels=3, nr_logistic_mix=15, num_classes=4, embedding_dim=80)
    model_path = os.path.join(os.path.dirname(__file__), "models","pcnn_cpen455_from_scratch_119_condition_batch_16_resnet_3_filter_100_mix_15.pth")
    print(model_path)
